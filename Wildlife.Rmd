...
cover <- data.frame(dcast(mabuiaMeio, site ~ Date, value.var = "Landscape")[,2])
names(cover) <- "cover"
...
pop[, areaM2 := (pi*3^2)] # pi*observation radius^2 = observed area
pop[, areaHa := areaM2/10^4] # area in hectare
...
# each covariate. 
# Examples are: Cover (landscape type)
cover <- data.frame(dcast(mabuiaRata, site ~ Date, value.var = "Landscape")[,2]) ##### Un-hardcode!
names(cover) <- "cover"
...
################################# ILHA DO MEIO ELENIA
# First we subset the data to the interested place and the species
eleniaMeio <- DT[Island == "Ilha do Meio" & Species == "Elaenia ridleyana",]
...
toRemove <- c("Island", "Year", "Month", "Day", "Species", 
              "Radius", "Original_Landscape", "originDate",
              "pointID", "transectID")
eleniaMeio <- eleniaMeio[, (toRemove) := NULL]
...
# (increasing) sampling occasion (i.e., the time series of counts, representing 
# each visit), which unmarked calls 'T'. The value is the Counts.
finalCounts <- dcast(eleniaMeio, site ~ Date, value.var = "Counts")
y <- as.matrix(finalCounts[,-1]) # Convert to matrix
...
# representing each site and the columns each sampling occasion. 
# Examples are: observerID, JulianDate, timeSinceStartEradication
observerID <- as.matrix(dcast(eleniaMeio, site ~ Date, value.var = "Observer")[,-1])
JulianDate <- as.matrix(dcast(eleniaMeio, site ~ Date, value.var = "JulianDate")[,-1])
timeSinceStartEradication <- as.matrix(dcast(eleniaMeio, site ~ Date, value.var = "timeSinceStartEradication")[,-1])
# Fill in the matrix of JulianDate and timeSinceStartEradication: this affects the models later on
replaceNAwithColMean  <- function(mx){
  DT <- as.data.table(mx)
  nm <- names(DT)[colSums(is.na(DT)) != 0]
...
obsCovs <- list(#observerID = observerID, # Observer is a moot point for Elenia, as only one person did it
               JulianDate = JulianDate,
               timeSinceStartEradication = timeSinceStartEradication)
...
# each covariate. 
# Examples are: Cover (landscape type)
cover <- data.frame(dcast(eleniaMeio, site ~ Date, value.var = "Landscape")[,2]) # Here we 
# get any column but the first, which is the site. 
names(cover) <- "cover" # Also a moot point for Elenia as they are only in the treed area
...
modList <- fitList(
  "lam(.)gamma(.)phi(.)p(.)" = nullP,
  "lam(.)gamma(timeSinceStartEradication)phi(.)p(.)" = mod1,
  "lam(.)gamma(.)phi(.)p(JulianDate)" = mod2,  # Best model
  "lam(.)gamma(timeSinceStartEradication)phi(.)p(JulianDate)" = mod3, 
  "lam(.)gamma(timeSinceStartEradication+JulianDate)phi(.)p(.)" = mod4, 
  "lam(.)gamma(timeSinceStartEradication+JulianDate)phi(.)p(JulianDate)" = mod5,  
  "lam(.)gamma(JulianDate)phi(.)p(JulianDate)" = mod6,  
  "lam(.)gamma(JulianDate)phi(.)p(.)" = mod7
)
