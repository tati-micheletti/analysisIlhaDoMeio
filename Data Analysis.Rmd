...
the table by eliminating the redundant values.
...
We then create Table 5, by concatenating the estimates values and confidence intervals.
...
table5 <- rbindlist(lapply(unique(finalTable[["species"]]), function(sp) {
  table5.isl <- rbindlist(lapply(unique(finalTable[["island"]]), function(isla) {
    table5.cyc <- rbindlist(lapply(unique(finalTable[["cycle"]]), function(cyc) {
      table5.par <- rbindlist(lapply(unique(finalTable[["parameter"]]), function(parm) {
      ...
      table5.par[, c("species", "island", "cycle") := list(sp, isla, cyc)]
      tabDC <- dcast(table5.par, species + island + cycle ~ parameter, value.var = "estimate")
      }))
  }))
}))
table5_Final <- table5[!with(table5,is.na(detectionProbability)),] # Removing rows without data
table5_Final[, species := factor(species, levels = c("Trachylepis atlantica",
                                                        "Johngarthia lagostoma",
                                                        "Elaenia ridleyana",
                                                        "Sula dactylatra"))]
table5_Final[, island := factor(island, levels = c("Ilha do Meio",
                                                   "Ilha Rata"))]
setkey(table5_Final, species, island, cycle)
# Set key on species and cycles
# write.csv(table5_Final, "outputs/table5.csv") # Writing the table5.csv file
```
...
For the manuscript, a table with the best models has also been created:
