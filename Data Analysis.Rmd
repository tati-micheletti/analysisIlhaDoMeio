...
All model formulation's tested for all species are shown on Table X in the manuscript. 
The best model for each species and island (i.e., Ilha Rata as Control, Ilha do Meio as Treatment) 
was chosen based on Akaike Information Criterion (AIC) and the predictions for population growth rate 
of Ilha do Meio for the best model were reduced by the predictions for population growth rate 
of Ilha Rata. This ensured that drivers which could affect population growth in general in the region
(i.e., extreme events, natural population variation cicles, among others) were eliminated. We then 
plotted the results of all species (i.e., black rat density, and differential population growth rate
 of mabuias, crabs, elaenia and masked booby) in the same plot, Figure X.  
...
After running the model, we built and saved a table showing  species, island, cycle (matching Table 
XX), site, landCover, variable, estimate, standardError, lower confidence interval (LCI), upper 
confidence interval (UCI), modelType, modelFormulation, AIC, K, totalModelRuntime.
...
to notice that the models for this species did not converge throwing the error `Hessian is singular.`. 
This generally happens when parameters are non-identifiable. 
...
crabMeio <- modelWildlife(DT = Data, island = "Ilha do Meio", 
                             ...
                             useImmigration = FALSE,
                             useRobustDesign = FALSE,
                          starts=c(4.307, 0.0963, -3.816, -2.47))
# Get for 
# ZIP:
# NB:
# P:
# each one of the existing models... Ugh. Or... just check if the model of interest ("lam(landscapeCover)gamma(.)p(totalRainfallThreeMonths+moonPhase)iota(.)")
# has different estimates than with the values. Maybe just implement for it?!
# starts <- c(4.307,-0.413,1.317,0.0963,-3.816,0.733,-0.394,-2.47)
```
